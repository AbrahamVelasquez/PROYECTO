...
<form method="GET" action="index.php" class="flex flex-col md:flex-row gap-4 mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100 items-center">
  <input type="hidden" name="accion" value="mostrarPanel">
  <input type="hidden" name="tab" value="2">
  <div class="flex-1 relative w-full">
    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm">🔍</span>
    <input type="text" name="busqueda" value="<?= htmlspecialchars($_GET['busqueda'] ?? '') ?>" placeholder="BUSCAR POR APELLIDOS O DNI..." class="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 bg-white text-[10px] font-bold outline-none focus:ring-2 focus:ring-orange-100 transition-all uppercase">
  </div>
  ...
  <div class="flex items-center gap-3 w-full md:w-auto">
    ...
    <select name="ordenar" class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-[10px] font-bold outline-none cursor-pointer uppercase">
        <option value="estado"         <?= ($_GET['ordenar'] ?? '') == 'estado'          ? 'selected' : '' ?>>ESTADO</option>
        <option value="nombre"         <?= ($_GET['ordenar'] ?? '') == 'nombre'          ? 'selected' : '' ?>>ALUMNO</option>
        <option value="mis_convenios"  <?= ($_GET['ordenar'] ?? '') == 'mis_convenios'  ? 'selected' : '' ?>>CONVENIO</option>
        <option value="fecha_inicio"   <?= ($_GET['ordenar'] ?? '') == 'fecha_inicio'   ? 'selected' : '' ?>>FECHA INICIO</option>
        <option value="fecha_final"    <?= ($_GET['ordenar'] ?? '') == 'fecha_final'    ? 'selected' : '' ?>>FECHA FINAL</option>
    </select>
  </div>
  ...
  <div class="flex items-center gap-3 w-full md:w-auto">
    ...
    <select name="estado" class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-[10px] font-bold outline-none cursor-pointer uppercase">
      ...
      <option value="SIN ASIGNAR" <?= ($_GET['estado'] ?? '') == 'SIN ASIGNAR' ? 'selected' : '' ?>>🔴 SIN ASIGNAR</option>
