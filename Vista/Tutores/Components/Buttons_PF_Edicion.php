...
window.exportarYMarcar = async function(idAsignacion) {
    ...
    const campos = {
        ...
        'fecha_plan': 'pf_edit_fecha_plan',
        'anio_inicio': 'pf_edit_anio_inicio',
        'anio_fin': 'pf_edit_anio_fin',
        'curso_selector': 'pf_edit_curso_selector',
        'nombre_ciclo': 'pf_edit_nombre_ciclo',
        'codigo_ciclo': 'pf_edit_codigo_ciclo',
        'email_alumno': 'pf_edit_email_alumno',
        'tel_alumno': 'pf_edit_tel_alumno',
        'centro_nombre': 'epf_dit_centro_nombre',
        'centro_correo': 'pf_edit_centro_correo',
        'centro_tel': 'pf_edit_centro_tel',
        'id_convenio': 'pf_pf_id_convenio',
...
window.guardarBorrador = async function(idAsignacion) {
    ...
    const formData = new URLSearchParams();
    formData.append('id_asignacion', idDefinitivo);
    formData.append('solo_borrador', '1'); // flag para distinguirlo en el backend
    ...
    try {
        ...
        const textoBruto = await res.text();
        const inicioJson = textoBruto.indexOf('{');
        if (inicioJson === -1) throw new Error("Respuesta no válida: " + textoBruto);

        const data = JSON.parse(textoBruto.substring(inicioJson, textoBruto.lastIndexOf('}') + 1));
