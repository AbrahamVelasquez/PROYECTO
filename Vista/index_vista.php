    ...
    <div class="flex justify-between items-start mb-10">
        ...
        <p class="mt-2 text-slate-500 text-sm leading-relaxed max-w-xl">
            Interfaz de gestión interna FFE. Por favor, selecciona una fase del flujo de trabajo para gestionar el registro, la asignación de plazas o el seguimiento.
        </p>
        </div>

        <div class="relative ml-4">
        <button id="userMenuBtn" class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-2 pr-4 hover:bg-slate-50 transition-all cursor-pointer shadow-sm">
            ...
            <svg class="h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 9l-7 7-7-7" stroke-width="2"/></svg>
        </button>
        ...
        <div id="userDropdown" class="hidden absolute right-0 mt-2 w-72 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
            ...
            <p class="text-[11px] font-bold text-slate-800 uppercase italic">
                <?php 
                // Lógica de conversión de "Primero/Segundo" a "1º/2º"
                $cursoLimpio = mb_strtolower(trim($cursoTutor));
                $abreviatura = $cursoTutor; // Por si no coincide con ninguno

                if (str_contains($cursoLimpio, 'primero')) {
                    $abreviatura = "1º";
                } elseif (str_contains($cursoLimpio, 'segundo')) {
                    $abreviatura = "2º";
                }
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-8 relative mb-8">
            ...
            <div class="step-label relative z-10 flex flex-col items-center text-center" data-step="<?= $num ?>">
                <div class="step-circle flex h-6 w-6 items-center justify-center rounded-full bg-slate-200 transition-all duration-300 mb-3 border-2 border-white shadow-sm">
        ...
        <section class="mt-8 rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden min-h-[500px]">
        <div class="p-10">
            ...
            <div data-tab="1" class="<?= $pestanaActiva == 1 ? 'active' : '' ?>">
                ...
                <form action="index.php" method="GET" class="flex gap-3 w-full mb-10">
                    ...
                    <button type="submit" class="bg-[#1a1f2e] text-white px-10 py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg cursor-pointer">Buscar</button>
                </form>
                ...
                <div class="mt-12">
                    <h3 class="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">Mi Listado Personal</h3>
                    <div class="overflow-hidden rounded-2xl border-2 border-orange-100 shadow-sm bg-white">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-orange-50">
                                <?php if (!empty($misConvenios)): ?>
                                    <?php foreach ($misConvenios as $mc): ?>
                                        ...
                                        <td class="px-6 py-5 text-center">
                                            ...
                                            <form action="<?= $urlDestino ?>" method="POST">
                                                ...
                                                <button type="button" onclick="abrirConfirmarEliminar(<?= $mc['id_convenio'] ?>, '<?= htmlspecialchars($mc['nombre_empresa']) ?>')"
                                                        class="group flex items-center gap-2 mx-auto bg-red-50 hover:bg-red-500 text-red-500 hover:text-white px-4 py-2 rounded-lg transition-all border border-red-100 shadow-sm">
                                                    <span class="text-[10px] font-black uppercase tracking-tighter">Eliminar de la lista</span>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 border border-slate-100">
        ...
        <button onclick="document.getElementById('modalConvenioEnUso').style.display='none'"
                class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-xs font-bold hover:bg-slate-700 transition-all cursor-pointer">
            Entendido
        </button>
        </div>
    </div>
    ...
    <div id="modalConfirmarEliminar" style="display:none"
        class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
        onclick="if(event.target===this) this.style.display='none'">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 border border-slate-100">
        ...
        <button onclick="document.getElementById('modalConfirmarEliminar').style.display='none'"
                class="text-slate-400 hover:text-slate-700 text-xl font-bold leading-none cursor-pointer">✕</button>
        </div>
        ...
        <button id="btnConfirmarEliminarFav"
                class="px-5 py-2.5 rounded-xl bg-red-500 text-white text-xs font-bold hover:bg-red-600 transition-all cursor-pointer">
            Sí, eliminar
        </button>
        </div>
    </div>
